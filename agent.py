...
from tools.summary import make_summary_tools

load_dotenv()


def build_kora(token: str, role: str):
    today    = date.today()
    ...
    summary_tools  = make_summary_tools(token)

    all_tools = booking_tools + employee_tools + leave_tools + summary_tools + [notify_staff]
...
QUICK ACTIONS — work for both roles:
Daily Summary      → ALWAYS call get_daily_summary
Upcoming Schedule  → ALWAYS call get_upcoming_schedule
Today's Agenda     → ALWAYS call get_todays_agenda
Team Performance   → ALWAYS call get_team_performance
Check in           → ALWAYS call check_in_attendance
Check out          → ALWAYS call check_out_attendance
        ...
        def invoke(self, inputs: dict) -> dict:
            ...
            while hasattr(response, "tool_calls") and response.tool_calls:
                ...
                for tool_call in response.tool_calls:
                    ...
                    if tool_fn:
                        try:
                            result = tool_fn.invoke(tool_args) if tool_args else tool_fn.invoke({})
