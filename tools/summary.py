...
#         """
#         Get a complete daily summary - appointments today, attendence status , and leave balance.
#         Use this for the Daily Summary quick action.
#         No input needed.
...
#         # Appointments today
#         appt_result = api_client.api_get(
#             "/api/v1/appointment/",
#             token,
#             params={"status": "all"}
#         )
#         if "error" not in appt_result:
#             all_appts = appt_result.get("data") or []
#             if isinstance(all_appts, dict):
#                 all_appts = all_appts.get("data") or []
#             todays  = [a for a in all_appts if (a.get("appointmentDate", a.get("date", "")) or "")[:10]==today]
#             upcoming = [a for a in todays if a.get("status", "").lower() == "upcoming"]
#             summary.append(f"Appointments today: {len(todays)} total, {len(upcoming)} upcoming")
#         else:
#             summary.append("Appointments: could not load")
...
#             if balances:
#                 parts = [f"{lt}: {v.get('remaining', 0)} days left" for lt, v in balances.items()]
...
#         Use this for the Upcoming Schedule quick action.
#         No input needed.
#         """
...
#             appt_date = (a.get("appointmentDate", a.get("date", "")) or "")[:10]
#             lines.append(
#                 f"  • {appt_date} | {a.get('startTime','N/A')} - {a.get('endTime','N/A')}"
...
#         """
#         Get the full agenda for today — appointments and attendance.
#         Use this for the Today's Agenda quick action.
#         No input needed.
#         """
#         today  = date_lib.today().strftime("%Y-%m-%d")
...
#             output.append(f"\nAttendance: {status} | In: {check_in} | Out: {check_out}")
...
#         Use this for the Team Performance quick action.
#         No input needed.
#         """
...
#         if not lines:
#             return f"Performance data received but could not be formatted.\nRaw: {str(data)[:300]}"
...
#         """
#         Check in for today (mark attendance started).
#         No input needed.
#         """
...
#         """
#         Check out for today (mark attendance ended).
#         No input needed.
#         """
#         BASE_URL = os.getenv("BASE_URL", "")
#         try:
